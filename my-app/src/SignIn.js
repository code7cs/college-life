...
class SignUp extends Component{
    ...
    handleSubmit = async event=>{
        ...
        try{
            let registerUser = await firebase.auth().signInWithEmailAndPassword(this.state.email, this.state.password);
    ...
    render() {
        ...
        <div className = "container">
         <form onSubmit={this.handleSubmit}>
            <label> Email : </label><input type="text" value={this.state.email} name="email" onChange={this.handleChange}/>
            <br/><label> Password : </label><input type="password" value={this.state.password} name="password" onChange={this.handleChange}/>
            <br/><input className="btn btn-primary" type="submit" value="SignIn"/>
            <br/>Not a Member? <a href="/signup"> SignUp </a>
         </form>
         {error}
        </div>
        )
